...
pub fn process_generics(def: &mut Def) -> syn::Result<Vec<ResultOnEmptyStructMetadata>> {
	...
	for storage_def in def.storages.iter_mut() {
		...
		if let Some(named_generics) = storage_def.named_generics.clone() {
			...
			// Add the `PovEstimation` generic.
		}
		use crate::pallet::parse::storage::PovEstimationAttr;
		args.args.push(match storage_def.pov_estimation {
			Some(PovEstimationAttr::Measured) =>
				syn::parse_quote!(frame_support::storage::PovEstimationMeasured),
			Some(PovEstimationAttr::MaxEncodedLen) =>
				syn::parse_quote!(frame_support::storage::PovEstimationMel),
			None => syn::parse_quote!(frame_support::storage::PovEstimationNone),
		});
	}
